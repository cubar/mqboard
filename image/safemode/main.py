...
u = os.uname()
log.info(f'MicroPython {u.release=}_{u.version=} {u.machine=}')
del u
...
if sys.platform == "esp32":
    ...
    if min_idf_heap or max_mp_heap:
        try:
            from esp32 import NVS, idf_heap_info  # noqa
            nvs = NVS("micropython")
            reset=False
            # minimum IDF heap size
            if min_idf_heap:
                try: mih = nvs.get_i32("min_idf_heap")  # noqa
                except: mih = None  # noqa
            ...
            if max_mp_heap:
                try: mmh = nvs.get_i32("max_mp_heap")  # noqa
                except: mmh = None  # noqa
...
def main():  # function keeps global namespace clean
    ...
    if hasattr(board, "modules") and board.modules:
        ...
        from esp32 import idf_heap_info, HEAP_DATA  # noqa
        ...
        def lm():
            #log.info("MEM free=%d", gc.mem_free())  # no mem_maxfree attr in v1.18 yet
            log.info(f'{gc.mem_free()=}')

        loop.set_exception_handler(def_exception_handler)
        lm()

        # the loader task iterates through the modules (typ from config) and starts each one
        async def loader():
            for name in board.modules:
                try:
                    ...
                    if not fun:
                        ...
                        log.info(f'load {name} config: [%s]', ", ".join(iter(config)))
